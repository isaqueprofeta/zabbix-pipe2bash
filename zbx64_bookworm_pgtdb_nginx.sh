...
sudo apt-get -q -y install curl ca-certificates gnupg2
...
echo "########################################################"
wget --quiet https://repo.zabbix.com/zabbix/6.4/debian/pool/main/z/zabbix-release/zabbix-release_6.4-1+debian12_all.deb
...
echo "########################################################"
sudo apt-get -q -y install zabbix-frontend-php php8.2-pgsql zabbix-nginx-conf

echo "########################################################"
echo "ZABBIX FRONTEND - Configurando php"
echo "########################################################"
sed -i "s/php_value\[date.timezone\] = #{PHP_TIMEZONE}/php_value\[date.timezone\] = America\/Sao_Paulo/" /etc/php/8.2/fpm/pool.d/zabbix-php-fpm.conf
